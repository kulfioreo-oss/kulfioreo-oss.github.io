...
export class EndingScreen extends Phaser.Scene {
    ...
    create() {
        ...
        this._buildScorePanel(W, H);

        this._createButton(W / 2 - 135, 640, 'MAIN LAGI', () => {
        ...
        this._createButton(W / 2 + 135, 640, 'SELESAI', () => {
    ...
    _buildScorePanel(W, H) {
        const remainingHealth = this._score.remainingHealth ?? 0;
        const maxHealth = this._score.maxHealth ?? 5;
        const mistakes = this._score.mistakes ?? Math.max(0, maxHealth - remainingHealth);

        this.add.text(W / 2, 522, 'Skor Akhir', {
            fontFamily: '"Courier New", Courier, monospace',
            fontSize: '18px',
            color: '#FFD700',
            stroke: '#000000',
        ...
        this.add.text(W / 2, 552, this._getStarText(), {
            fontFamily: '"Courier New", Courier, monospace',
            fontSize: '24px',
            color: '#FFD700',
            stroke: '#000000',
        ...
        this.add.text(W / 2, 594, `Kesalahan: ${mistakes}    Health tersisa: ${remainingHealth}/${maxHealth}`, {
            fontFamily: '"Courier New", Courier, monospace',
            fontSize: '14px',
            color: '#FFFFFF',
            stroke: '#000000',
    ...
    _getStarRating() {
        const remainingHealth = this._score.remainingHealth ?? 0;
        return Math.max(1, Math.min(5, remainingHealth));
    }

    _getStarText() {
        const stars = this._getStarRating();
        return `${'* '.repeat(stars)}${'- '.repeat(5 - stars)}`.trim();
    }
